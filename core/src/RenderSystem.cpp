...
namespace core{
	 ...
	 *	- Refactor init
	 *	- Move calculation of model matrix to the more appropriate place (e.g. in render loop)
	 *	- And do the calculation in more appropriate way (e.g. we don't need to call glm::mat4(1) every time)
	...
	int RenderSystem::initGLFW(){
		...
		// glfw window creation
		// --------------------
		_window = glfwCreateWindow(_scr_width, _scr_height, "RenderWindow", NULL, NULL);
		if (_window == NULL) {
			std::cout << "Failed to create GLFW window" << std::endl;
			glfwTerminate();
			return 0;
